    ...
    var init = function(){
        ...
        $(document).on('click','a',function(e){
            var target = $(this).data('target'),
                href = $(this).attr('href');
            if(!href ||  href.match(/^#/) || (target&&target!="link")){
                e.preventDefault();
                return false;
            }
        });
...
J.Template = (function($){
    ...
    /**
     * 加载等待背景模板
     * @param templateId handlbars模板ID
     */
    
    var parseTemp = function(templateId) {
    ...
    var render = function(containerSelector,templateId,data,type){
        ...
        if($.type(data) == 'array' && data.length == 0 ){
            ...
            var template = Handlebars.compile(parseTemp(templateId))
...
J.Popup = (function($){
    var _popup,_mask,transition,clickMask2close,
        ...
        TEMPLATE = {
            alert : '<div class="popup-title">{title}</div><div class="popup-content">{content}</div><div id="popup_btn_container" data-selected="selected"><a data-target="closePopup" data-icon="checkmark" data-selected="selected">{ok}</a></div>',
            confirm : '<div class="popup-title">{title}</div><div class="popup-content">{content}</div><div id="popup_btn_container" data-selected="selected"><a class="cancel" data-icon="close" data-selected="selected">{cancel}</a><a data-icon="checkmark" data-selected="selected">{ok}</a></div>',
    ...
    var show = function(options){
        ...
        if(settings.html){
            ...
            var template = Handlebars.compile(J.Template.parseTemp(settings.tplId))
            html = template(settings.tplData);
        }
     ...
     */
    var popover = function(obj, pos,arrow_direction,onShow){
        var html = "", tplId = null;
        if($("#"+obj.el).length) {
            tplId = obj.el
        } else {
        ...
        show({
            tplId: tplId,
            tplData: obj.data ? obj.data : null, 
            html : html,
            pos : pos,
            showCloseBtn : false,
